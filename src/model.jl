...
`DevTables`) and "a thing a solver can consume" (a QuickPOMDP). Keeping the solver out of
...
function build_pomdp(config::StationkeepingPOMDP = StationkeepingPOMDP();
    ...
    return QuickPOMDP(
        ...
        isterminal    = s -> isterminal_dev(s.dev),

        # Start on the nominal orbit, no science banked yet.
        initialstate  = Deterministic(SKState(:OK, 0)),

        transition    = (s, a)  -> SparseCat(S, T[sidx[s], aidx[a], :]),

        # The observation depends only on the landed state's dev bin.
