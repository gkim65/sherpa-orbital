...
altitude, as a softmax over normalized band depth:

    P_θ(ℓ | b)  ∝  exp( θ · d_b · z_ℓ )

with `d_b` the normalized band depth in [0, 1] (1 = lowest-altitude bin) and `z_ℓ` the
centered level score in [-1, 1]. Softmax rather than a linear tilt because θ = 0 is then
exactly uniform for every band by construction — the sanity gate is structural, not tuned —
and because it stays a valid distribution for every θ ≥ 0 with no clipping, so no two θ
collapse onto the same POMDP.
...
`P_θ(ℓ | bin)` over the `plume_levels` intensity levels, from the softmax in this file's
header.
...
intensity in deep bins and low intensity in shallow ones.
"""
